          ...
          {tasks.length === 0 ? (
            ...
            >
              <div className="relative" style={{ width: 'min-content' }}>
                  ...
                  {/* Body */}
                  <div className="relative" style={{ gridTemplateRows: `repeat(${sortedTasks.length}, 1fr)` }}>
                      {/* Vertical Grid Lines for Timeline */}
                      <div className="absolute inset-0 z-0">
                          {viewMode === 'day' ? (
                              days.map((day, index) => (
                                  <div key={`v-line-${index}`} className="absolute top-0 h-full w-px bg-transparent border-l border-dashed border-border/80" style={{ left: `${(index * DAY_WIDTH_DAY_VIEW) + DAY_WIDTH_DAY_VIEW - 1 + TASK_LIST_WIDTH}px` }} />
                              ))
                          ) : (
                              weeks.map((week, index) => (
                                  <div key={`v-line-${index}`} className="absolute top-0 h-full w-px bg-transparent border-l border-dashed border-border/80" style={{ left: `${(index * WEEK_WIDTH) + WEEK_WIDTH - 1 + TASK_LIST_WIDTH}px` }} />
                              ))
                          )}
                      </div>
                      ...
                      {(() => {
                          ...
                          return (
                              <div ref={todayRef} className="absolute top-0 bottom-0 w-0.5 bg-primary z-20" style={{ left: `${todayLeft + TASK_LIST_WIDTH}px` }} >
                                  <div className="sticky top-0 left-1/2 -translate-y-full -translate-x-1/2 bg-primary text-primary-foreground text-xs font-bold px-2 py-1 rounded-t-md">
                                      Today
                                  </div>
                              </div>
                          );
                      ...
                      {sortedTasks.map((task, index) => {
                          ...
                          return (
                              <div key={task.id} className="flex border-b" style={{ height: `${ROW_MIN_HEIGHT}px`}}>
                                  <div className="sticky left-0 bg-card border-r flex flex-col justify-center px-2 py-1 w-full" style={{ width: `${TASK_LIST_WIDTH}px` }}>
