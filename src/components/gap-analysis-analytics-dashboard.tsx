...
export function GapAnalysisAnalyticsDashboard({ records }: GapAnalysisAnalyticsDashboardProps) {
  const analyticsData = useMemo(() => {
    ...
    const countBy = (key: keyof GapAnalysisRecord) => records.reduce((acc, record) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="lg:col-span-2">
            <CardHeader>
                <CardTitle>Top Annexes</CardTitle>
                <CardDescription>Distribution of records per ICAO Annex.</CardDescription>
            </CardHeader>
            <CardContent className="pl-2">
                <ChartContainer config={chartConfig(analyticsData.annexData)} className="h-[300px] w-full">
                    <ResponsiveContainer>
                        <BarChart data={analyticsData.annexData.slice(0, 10)} layout="vertical" margin={{ left: 50, right: 30 }}>
