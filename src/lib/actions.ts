...
import { ccefodFormSchema, pqFormSchema } from './schemas';

const window = new JSDOM('').window;
const purify = DOMPurify(window as any);

// --- SHARED SCHEMAS ---
const ccefodRecordSchema = z.object({
...
export async function addCcefodRecord(data: unknown) {
    const parsed = ccefodRecordSchema.safeParse(data);
    if (!parsed.success) {
...
export async function addRulemakingProject(projectData: unknown) {
  // We use `any` here because the full zod schema is now in the client component
  // and we don't want to import it into this server file.
  // The client component is responsible for validation.
